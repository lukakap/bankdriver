          ...
          int numWorkers)
{
  ...
  bank->next = malloc(numWorkers * sizeof(sem_t));
  
  int i = 0;
  for(i = 0; i < numWorkers; i++){
    sem_init(&(bank->next[i]),0,0);
  }

  bank->nWorkers = numWorkers;

  sem_init(&(bank->check),0,1);
  sem_init(&(bank->bankReport),0,1);

  bank->inProcessWorkers = numWorkers;
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for (unsigned int index = 0; index < bank->numberBranches; index++) {
        sem_post(&(bank->branches[index].brancLock));
      }
      return err;
    }
  ...
  *balance = bankTotal;
  ...
  for (unsigned int nOfBranch=0; nOfBranch < bank->numberBranches; nOfBranch++) {
    sem_post(&(bank->branches[nOfBranch].brancLock));
  }
