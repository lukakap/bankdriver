...
// void helper(Bank *bank, int workerNum){
//   for (int i = 0; i < bank->nWorkers; i++) {
//     if (i != workerNum){
//       sem_post(&(bank->next[i]));
//     }
//   }  
//   bank->inProcessWorkers = bank->inProcessWorkers;
//   sem_post(&(bank->check));
// }
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  sem_wait(&(bank->check));
  bank->inProcessWorkers--;
  if(bank->inProcessWorkers==0){
    Report *rpt = bank->report;
    assert(rpt); Y;

    if (rpt->numReports >= MAX_NUM_REPORTS) {
      //We've run out of report storage for the bank
      for(int i=0; i<bank->inProcessWorkers; i++){
        if(i!=workerNum){
          sem_post(&(bank->next[i]));
        }
      }

      bank->nWorkers = bank->inProcessWorkers;
      sem_post(&(bank->check));
    //  helper(bank, workerNum);
